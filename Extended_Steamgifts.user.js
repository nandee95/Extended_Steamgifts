...
function display_options() {
	...
	function addToOptions(name, save, def) {
		...
		content += `<div class="form__row" value="` + val + `" key=` + save + `>
...
if ((path == '/' || path == "/giveaways/") && Number(GM_getValue("esg_autoscroll", 1))) {
	if ($(".page__heading__breadcrumbs:contains('Active Discussions')").length) {
		...
		$(".sidebar__navigation:last").after(`<h3 class="sidebar__heading">Active Discussions</h3>
			<ul class="sidebar__navigation">
			` + c1 + `
	...
	if ($(".page__heading__breadcrumbs:contains('Community Voted')").length) {
		...
		.find(".table__rows").find(".table__row-outer-wrap").each(function () {
			...
			<i class="global__image-outer-wrap global__image-outer-wrap--game-small"><div class="global__image-inner-wrap" style="background-image:url(` + img + `);"></div></i>
			<div class="sidebar__navigation__item__underline">
			...
			` + votes + ` votes <br>` + percent + `%
			</div>
			<form>` + form + `</form>
			</div>
			</li>`;
		});
		$(".sidebar__navigation:last").after('<h3 class="sidebar__heading">Community Voted</h3><ul class="sidebar__navigation">' + c2 + '</ul>');
	}
			...
			$.ajax({
				...
				success : function (source) {
					...
					if (path.match('^/giveaways/') || path == "/") {
						...
						mainurl = pageurl.substring(0, pageurl.indexOf('&'));
...
// SGE menu
$('.nav__button[href|="/about/faq"]').closest('.nav__button-container').before(`<div class="nav__button-container">
...
$(".footer__outer-wrap").prepend(`<div style="z-index: 9999" class="popup__desc-loading popup">
